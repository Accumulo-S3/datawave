...
/**
 * The scope of test class aims to test only the scenarios in which EventMapper encounters an error, to verify its
 * behavior with the FieldSalvager interface.
 */
public class EventMapperSalvageFieldsOnErrorTest {
    ...
    public void setupTest(String dataTypeHandler) throws Exception {
        ...
        Type type = setupTypeAndTypeRegistry(dataTypeHandler);
        record = EventMapperTest.createBasicRecord(eventTime, type);
    }

    private Type setupTypeAndTypeRegistry(String dataTypeHandler) {
    ...
    /**
     * FakeSalvagingIngestHelper:
     * - always throws an exception when getEventFields is called, to ensure error handling code path is reached within EventMapper.
    ...
    /**
     * SalvagingDataTypeHandler provides a FieldSalvager implementation that deserializes rawData as a Map&lt;String, String&gt;,
     * then returns a Multimap containing only the SALVAGED_FIELDS that were encountered, skipping all others.
     */
    ...
    public void shouldSalvageAllFields() throws Exception {
        ...
        record.setRawData(mapToRawDataBytes(fieldValues));
    ...
    private byte[] mapToRawDataBytes(HashMap<String, String> map) throws IOException {
