...
ps -efww | egrep "python2 .*cleanup-server.py.*" | grep -v grep | sed "s/[[:graph:]]* *\([[:digit:]]*\).*/$host: \1: Cleanup Server/"
./list-loader.sh
ps -efww | egrep "python2 .*_flag_maker.py.*" | grep -v grep | grep -v sed | sed "s/[[:graph:]]* *\([[:digit:]]*\).*\/ingest\/\(.*\)_flag_maker.py.*/$host: \1: Flag Maker: \2/"
