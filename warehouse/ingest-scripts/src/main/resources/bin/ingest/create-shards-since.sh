...
for table in "${TABLES[@]}" ; do
   $WAREHOUSE_ACCUMULO_BIN/accumulo -add $ADDJARS datawave.ingest.util.GenerateShardSplits $date $count ${NUM_SHARDS} $shardsPerSplit -addShardMarkers $USERNAME $PASSWORD ${table} $WAREHOUSE_INSTANCE_NAME $WAREHOUSE_ZOOKEEPERS
done
